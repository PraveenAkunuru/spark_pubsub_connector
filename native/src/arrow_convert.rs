...
#[allow(deprecated)] // MapBuilder is technically in beta/deprecated in some versions but needed for Map support? 
// Actually Arrow-rs >= 50 uses MapBuilder. Let's check imports.
// Wait, we need `MapBuilder` from `arrow::array`.
// Using `GenericMapBuilder` with `StringBuilder` keys and values.

use arrow::array::{ArrayRef, BinaryBuilder, StringBuilder, TimestampMicrosecondBuilder, MapBuilder};
...
impl ArrowBatchBuilder {
    ...
    pub fn finish(&mut self) -> (Vec<ArrayRef>, SchemaRef) {
        ...
        (
            vec![message_id_array, publish_time_array, payload_array, ack_id_array, attributes_array],
            Arc::new(schema)
        )
    }
}
